...
function Map({ origin, destination, apiKey }) {
  const { animal_name, setAnimal_name ,userProfileImage,userId,username,animalUrl,setAnimalUrl,animalType,setAnimalType} = useContext(userContext);
  ...
  const center = bounds.getCenter();
      ...
      markers.forEach((marker) => {
        ...
        const distances = markers.map((marker) => {
          const latDiff = marker.position.lat - origin.lat;
          const lngDiff = marker.position.lng - origin.lng;
          const distance = Math.sqrt(latDiff * latDiff + lngDiff * lngDiff);
          return distance;
        });
        
        const minIndex = distances.indexOf(Math.min(...distances));
        setAnimal_name(markers[minIndex].title);
        setAnimalUrl(markers[minIndex].realimageUrl)
        setAnimalType(markers[minIndex].type); 
